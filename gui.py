...
ALGORITHMS = ["FIFO","RR","MLT","EDF","DMA","RMA"]

class Application:
    def __init__(self,root:Tk): #Type hint
    ...
    def configWindow(self,window:Tk):
        window.title("Task Schedular")
        window.geometry('1000x800')

    def buildConfigFrame(self,parent):
        self.configFrame = LabelFrame(parent,text="Scheduling Policy Details",bg="#7469B6")

        self.algoSelLbl = Label(self.configFrame,text="Policy")
        ...
        self.taskCountLbl = Label(self.configFrame,text="Number Of Tasks")
        ...
        self.showTaskTitle = ttk.Checkbutton(self.configFrame,text="View Job Number",variable=self.showTitleVar)
        ...
        Label(self.configFrame,text="Timeline Length").grid(row=1,column=0,padx=(10,5),pady=(10,10))
        self.sysWatch.grid(row=1,column=1,padx=(5,10),pady=(10,10))

        Label(self.configFrame,text="Time Slot").grid(row=1,column=2,padx=(10,5),pady=(10,10))
    ...
    def buildTaskDefintionFrame(self,parent):
        ...
        self.taskDefLblFrame = LabelFrame(parent,text="Tasks Info")
        self.taskDefLblFrame.grid(row=1,column=0,columnspan=5)

        for i in range(task_count):
            lblFrame = LabelFrame(self.taskDefLblFrame,text=f"Task {i+1}")
        ...
        self.actionFrame = LabelFrame(self.taskDefLblFrame,text="Options")
        ...
        self.firstTestBtn = ttk.Button(self.actionFrame,text="Bound Test",padding=(10,8),command=lambda:self.applyFirstTest(parent))
        self.secondTestBtn = ttk.Button(self.actionFrame,text="Completion-Time Test",padding=(10,8),command=lambda:self.applySecondTest(parent))
    ...
    def applyFirstTest(self,parent):
        ...
        if result[0]:
            #Scheduable
            Label(self.testResultFrame,text=f"Accumulative utilization ( {result[1]} ) < Bound ( {result[2]} )").pack(padx=10,pady=10)
    ...
    def applySecondTest(self,parent):
        ...
        if firstRes : 
            Label(self.testResultFrame,text=f"Bound test indicates that tasks are scheduable").pack(padx=10,pady=10)
        else:
            Label(self.testResultFrame,text="Applying Completion time Test ...").pack(padx=10,pady=10)
            tasks = self.getTasks()
...
def draw_timing_diagram(logs:list,tasks:list[MyTask],showTaskTitle = True):
    ...
    for log in logs:
        ...
        if showTaskTitle:
            ax.text(x+((log[2]-log[1])/2), y+1.5, f"J {log[0].id}{log[0].jobNum}",
        ...
        if log[3] == "DEADLINE":
            ...
            ax.text(x+((log[2]-log[1])/2), 1.5, f"J {log[0].id}{log[0].jobNum}\ndeadline\nbroken",
